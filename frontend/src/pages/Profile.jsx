...
const Profile = () => {
    ...
    const fectchUser = async (profileId) => {
      const token = await getToken()
      try {
        const {data} = await APIKeys.post('/api/user/profile', {profileId},{
    ...
    useEffect(()=>{
       if(profileId){
        fectchUser(profileId)
       }else{
         fectchUser(currentUser._id)
       }
        ...
        <div className='mt-6'>
            <div className='bg-white rounded-xl shadow p-1 flex max-w-md mx-auto'>
             {["posts", "media", "likes"].map((tab)=>(
                <button onClick={()=> setActiveTab(tab)} key={tab} className={`flex-1d px-4 py-2 text-sm font-medium
                ...
                <div className='flex flex-wrap mt-6 max-w-6xl'>
                 {
                    posts.filter((post)=>post.image_urls.length > 0).map((post)=>(
                        ...
                        {post.image_urls.map((image, index)=>(
                          <a target='_blank' rel='noreferrer' href={image} key={index}
