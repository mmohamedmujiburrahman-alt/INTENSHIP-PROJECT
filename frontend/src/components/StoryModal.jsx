...
const StoryModal = ({setShowModal, fetchStories}) => {
    ...
    const handleMediaUpload = (e)=>{
        ...
        if(file){
            if(file.type.starsWith("video")){
                if(fiel.size > MAX_VIDEO_SIZE_MB * 1024 *1024){
            ...
            }else if(file.type.starsWith("image")){
                setMedia(file)
                setPreviewUrl(URL.createObjectURL(file))
                setText(' ')
                setMode("media")
    ...
    const handleCreateStory = async () => {
        const media_type = mode === 'media' ? media?.type.starsWith('image') ?
        'image' : "video" : "text";
                ...
                <div className='text-center mb-4 flex items-center justify-between'>
                 ...
                 relative' style={{backgroundColor: background}}>
                    ...
                    {mode === 'text' && (
                        ...
                        on your mind?" onChange={(e)=>setText(e.target.value)} value=
                        {text} />
                    )} 
                    {
                        mode === 'media' && previewUrl && (
                            media?.type.starsWith('image') ? (
                                <img scr={previewUrl} alt="" className='object-contain
                                max-h-full' />
                            ) : (
                ...
                <div className='flex gap-2 mt-4'>
                    <button onClick={()=> {setMode('text'); setMedia(null);
                     setPreviewUrl(null)}} className={`flex-1 flex items-center justify-center gap-2
                        p-2 rounded cursor-pointer ${mode === 'text' ? "bg-white text-black" : 
                       "bg-zinc-800"}`}>
                  ...
                  <button onClick={()=> toast.promise(handleCreateStory(), {
                    loading: 'Saving...', })} className='flex items-center justify-center gap-2 text-white
